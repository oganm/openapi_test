  ...
  public = list(
    ...
    fromJSON = function(input_json) {
      ...
      if (!is.null(this_object$`factorValuesUsed`)) {
        self$`factorValuesUsed` <- ApiClient$new()$deserializeObj(this_object$`factorValuesUsed`, "map(array[FactorValueValueObject])", loadNamespace("openapi"))
      }
      if (!is.null(this_object$`resultSets`)) {
        self$`resultSets` <- ApiClient$new()$deserializeObj(this_object$`resultSets`, "array[DiffExResultSetSummaryValueObject]", loadNamespace("openapi"))
      }
