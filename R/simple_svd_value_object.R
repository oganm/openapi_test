  ...
  public = list(
    ...
    fromJSON = function(input_json) {
      ...
      if (!is.null(this_object$`bioMaterialIds`)) {
        self$`bioMaterialIds` <- ApiClient$new()$deserializeObj(this_object$`bioMaterialIds`, "array[integer]", loadNamespace("openapi"))
      }
      if (!is.null(this_object$`variances`)) {
        self$`variances` <- ApiClient$new()$deserializeObj(this_object$`variances`, "array[numeric]", loadNamespace("openapi"))
      }
